...
export type ComboboxContextProps = {
  ...
  tagGroupRef: React.RefObject<React.ComponentRef<typeof RovingFocusGroupPrimitive.Root> | null>;
  disabled?: boolean;
  required?: boolean;
} & (
...
function Combobox<T extends ComboboxType = 'single'>({
  ...
  const [value, setValue] = useControllableState<ComboboxValue<T>>({
    ...
    defaultProp: ((defaultValue ?? type === 'multiple') ? [] : '') as ComboboxValue<T>,
  ...
  const tagGroupRef = React.useRef<React.ComponentRef<typeof RovingFocusGroupPrimitive.Root>>(null);
...
function ComboboxTagGroup({
  ...
  const { currentTabStopId, onCurrentTabStopIdChange, tagGroupRef, type } = useCombobox();
...
export interface ComboboxTagGroupItemProps
  extends Omit<React.ComponentProps<typeof RovingFocusGroupPrimitive.Item>, 'children'> {
...
function ComboboxTagGroupItem({
  ...
  const { value, onValueChange, inputRef, currentTabStopId, type } = useCombobox();
...
function ComboboxClear({ onClick, ...props }: React.ComponentProps<typeof Primitive.button>) {
  const { value, onValueChange, inputValue, onInputValueChange, type } = useCombobox();
...
function ComboboxList(props: React.ComponentProps<typeof CommandPrimitive.List>) {
...
function useComboboxItem() {
  ...
  if (!context) {
    throw new Error('<ComboboxItemContext> should only be used within a <ComboboxItem />.');
  }
...
function ComboboxItem({ value: valueProp, children, onMouseDown, ...props }: ComboboxItemProps) {
  const { type, value, onValueChange, onInputValueChange, onOpenChange } = useCombobox();
...
function ComboboxItemIndicator(props: React.ComponentProps<typeof Primitive.span>) {
  ...
  return <Primitive.span data-slot="combobox-item-indicator" aria-hidden {...props} />;
}

export interface ComboboxItemTextProps extends React.ComponentProps<typeof React.Fragment> {
  children: string;
}
...
function ComboboxAnchor(props: React.ComponentProps<typeof PopoverPrimitive.Anchor>) {
  return <PopoverPrimitive.Anchor data-slot="combobox-anchor" {...props} />;
}

function ComboboxPortal(props: React.ComponentProps<typeof PopoverPrimitive.Portal>) {
...
function ComboboxLoading(props: React.ComponentProps<typeof CommandPrimitive.Loading>) {
  return <CommandPrimitive.Loading data-slot="combobox-loading" {...props} />;
}

function ComboboxGroup(props: React.ComponentProps<typeof CommandPrimitive.Group>) {
  return <CommandPrimitive.Group data-slot="combobox-group" {...props} />;
}

function ComboboxSeparator(props: React.ComponentProps<typeof CommandPrimitive.Separator>) {
  return <CommandPrimitive.Separator data-slot="combobox-separator" {...props} />;
}
